export const cachedUnJSGitHubOrg = defineCachedFunction(async () => {
  const org = await $fetch<{ followers: number }>('https://api.github.com/orgs/uni-helper')

  return org
}, {
  maxAge: 1000 * 60 * 60 * 24, // 24 hours
  group: 'github',
  name: 'uniHelperGitHubOrg',
  getKey: () => 'uniHelperGitHubOrg',
})

export const cachedGitHubRepos = defineCachedFunction(async () => {
  const data = await $fetch<{ repos: { stars: number }[] }>('https://ungh.cc/orgs/uni-helper/repos')
...
export async function fetchStars(owner: string, repo: string): Promise<number> {
  ...
  console.log('fetchStars', owner, repo, metadata.stars)
